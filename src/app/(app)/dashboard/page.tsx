          ...
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-3">
            {saved.map((s) => {
              const budget = Array.isArray(s.cards_to_buy_budget)
                ? s.cards_to_buy_budget as Array<{ price_usd?: number; quantity: number }>
                : [];
                ...
                <Link key={s.id} href="/saved">
                  <div className="rounded-xl border border-border/60 bg-card p-4 cursor-pointer card-hover-glow transition-all duration-200">
                    ...
                    <p className="text-xs text-muted-foreground">
                      Budget: <span className="text-primary font-medium">${cost.toFixed(2)}</span> · {budget.length} kort att köpa
                    </p>
                  </div>
                </Link>
              );
